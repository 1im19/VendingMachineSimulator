...
void ExitVendingMachineProgram(ofstream &file, float totalsale, vendingmachine drink[])
{
    file.open("VendingMachine.txt");
    if (file.is_open())
    {
        file << "Total sale: RM" << totalsale << setprecision(2) << fixed << endl << endl;
        file << "Drinks that is still on stock: " << endl << endl;
        file << "Drink Name\tQuantity" << endl << endl;
        for (int i1 = 0; i1 < 14; i1++)
        {
            if (drink[i1].quantity > 0)
            {
                file << drink[i1].name << "\t" << drink[i1].quantity << endl;
            }
        }
        file << endl;
        file << "Drink that is being restocked: " << endl << endl;
        file << "Drink Name\tQuantity" << endl << endl;
        for (int i2 = 0; i2 < 14; i2++)
        {
            if (drink[i2].quantity < 10)
            {
                file << drink[i2].name << "\t" << 10 - drink[i2].quantity << endl;
            }
        }
        file.close();
    }
...
int main()
{
    ...
    do
    {
        ...
        {
            ExitVendingMachineProgram(file, totalsale, drink);
        }
        else
        {
